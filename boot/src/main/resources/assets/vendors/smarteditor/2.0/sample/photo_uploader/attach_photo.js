...
function goStartMode() {
	...
	if (sSrc.indexOf("btn_confirm2.png") < 0) {
        //facilityiot change start - change url of resource
		welBtnConfirm.attr("src", "/assets/vendors/smarteditor/sample/photo_uploader/img/btn_confirm2.png");
        //facilityiot change end
...
function goReadyMode() {
	...
	if (sSrc.indexOf("btn_confirm2.png") >= 0) {
        ...
        //facilityiot change start - change url of resource
		welBtnConfirm.attr("src", "/assets/vendors/smarteditor/sample/photo_uploader/img/btn_confirm.png");
        //facilityiot change end
	}
}
...
function addImage(ofile) {
	...
	if (!!bExceedLimitTotalSize) {
        ...
        //facilityiot change start - change url for resource
		aFileList.push('	<a onclick="delImage(\'img' + nImageInfoCnt + '\')"><img class="del_button" src="/assets/vendors/smarteditor/sample/photo_uploader/img/btn_del.png"  width="14" height="13" alt="첨부 사진 삭제"></a>');
        //facilityiot change end
		aFileList.push('	</li> ');
...
function html5Upload() {
	...
	// facilityiot changes start - change upload url
    var metas = document.getElementsByTagName('meta'),
        csrfValue = '';

    for (var i=0; i<metas.length; i++) {
        if (metas[i].getAttribute("name") === "_csrf") {
            csrfValue = metas[i].getAttribute("content");
            break;
        }
    }
    sUploadURL = '/common/editor/addEditorFiles.json?_csrf=' + csrfValue; //upload URL
	// facilityiot changes end
...
function makeArrayFromString(sResString) {
	...
	try {
		...
		// neoStory changes
		htTemp['sFileURL'] = sResString.substr(sResString.indexOf("=") + 1);
	} catch (e) {}
