var FindIdController = function (selector) {
    this.init(selector);
};

$.extend(FindIdController.prototype, {
    $container: null,
    moduleNo: 0,
    ...
    init: function (selector) {
        ...
        me.$form                    = me.$container.find('#form-findId-detail');
        me.$btnSave                 = me.$form.find('button.btn-findid-save');
        me.$btnCancel               = me.$form.find('button.btn-findid-cancel');
        me.$btnFindIdMailEditor     = me.$form.find('button.btn-findId-verificationMailEditor');
        me.$btnFindIdSMSEditor      = me.$form.find('button.btn-findId-verificationSMSEditor');
        //Checkboxs
    ...
    initEventHandlers: function() {
        ...
        me.$btnSave.on('click', function (e) {
            ...
            if(me.$chbMethodMemberPhone.is(':checked')
                ...
                mugrunApp.alertMessage('아이디찾기 방법을 1개 이상 선택하시기 바랍니다.');
            }
        });
        ...
        me.$btnFindIdMailEditor.on('click', function (e) {
            e.preventDefault();
            findIdIndexController.openFindIdVerificationMailEditorDialog();
        });

        me.$btnFindIdSMSEditor.on('click', function (e) {
            e.preventDefault();
    ...
    loadData: function(moduleNo) {
        ...
        $.ajax({
            url: '/admin/findid/loadConfigFindId.json',
            method: 'GET',
            dataType: 'json',
            data: {
                moduleNo: moduleNo
    ...
    submitFindIdConfig: function() {
        ...
        $.ajax({
            url: '/admin/findid/updateFindidMethod.json',
            type: 'POST',
            dataType: 'json',
